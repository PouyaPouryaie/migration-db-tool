...
| **drop-all** | Reset the database to "empty". | `docker run --rm -v $(pwd)/src/main/resources/db:/liquibase/db my-liquibase:latest --defaultsFile=/liquibase/db/local/liquibase-local.properties drop-all --force`    |
